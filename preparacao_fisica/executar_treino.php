...
input:placeholder-shown {
  background: #F5B7B1;
    ...
    <div id="page">
        ...
        <div class="col-12">
            <div class="container_form">
                ...
                echo '<form method="post" action="envia_treino.php" id="form1">';
                ...
                if(mysqli_num_rows($query)>0){
                    ...
                    while($exercicio = mysqli_fetch_assoc($query)){
                        ...
                        $n=1;
                        echo '<input type="number" style="display:none" name="id'.$i.'" id="id'.$i.'" value='.$exercicio['id'].'>';
                        echo '<input type="number" style="display:none" name="n_sr'.$i.'" id="n_sr'.$i.'" value='.$exercicio['n_series'].'>';
                        while($n<=$exercicio['n_series']){
                            $cargas[$treino['n_exercicios']-$i][$n]=str_replace(" ","+",$cargas[$treino['n_exercicios']-$i][$n]);
                            $tabela.="<td><input class='form-control' type='text' name='crg_".$i."_".$n."' id='crg_".$i."_".$n."' placeholder="."(".$cargas[$treino['n_exercicios']-$i][$n].")"." required></td>";                            
                        ...
                        echo $tabela;
                        echo '<div class="col-5" style="background-color:white" name="crn"></div>';
                        echo "</div></div>";
                        
                        if($i>1){echo '<button type="button" name="add_carga" id="volta_ex_'.$i.'" class="btn btn-primary" onclick="document.getElementById(\'ex_'.$i.'\').style.display=\'none\'; document.getElementById(\'ex_'. ($i-1) .'\').style.display=\'block\';">Exercício Anterior</button>';}
                    ...
                    echo '<input type="number" style="display:none" name="n_ex" id-"n_ex" value='.($i-1).'>';
                        
                }
                echo '<div id="pse" style="display: none">';
                    echo '<div class="row"><label for="ses">De 0(muito fácil) até 10(extremamente exaustivo), qual foi sua percepção de esforço no treino de hoje?</label><input type="number" min=0 max=10
                                    class="form-control" style="width:30%; margin-bottom:5px; margin-left: 18px;" name="ses" id="ses" required></div>
                            <div class="row"><label for="time">Quanto tempo levou o treino? (valor em minutos)</label><input type="number" class="form-control" style="width:30%; margin-bottom:5px; margin-left: 18px;" name="time" id="time" required></div>
                            <button type="button" class="btn btn-primary" onclick="document.getElementById(\'pse\').style.display=\'none\'; document.getElementById(\'ex_'.($i-1).'\').style.display=\'block\';">Voltar</button>
    ...
    <script src="../common/js/cronometro.js"></script>
